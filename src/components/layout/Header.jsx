...
export default function Header() {
  ...
  useEffect(() => {
    const height = showSearch ? "180px" : "110px";
  ...
  return (
    <header 
      onClick={unlockAudio} 
      className="fixed top-0 left-0 w-full z-50 bg-black transition-all duration-300 font-sans border-b border-zinc-900"
    >
      {/* Bungkus Top Bar dengan max-w-5xl agar tetap di tengah 
         sejajar dengan tabel di bawahnya saat layar lebar 
      */}
      <div className="max-w-5xl mx-auto w-full">
        <div className="flex items-center justify-between px-2 md:px-4 py-3 min-h-[60px] gap-2">
          ...
          <div className="flex items-center gap-1 sm:gap-3 flex-shrink-0">
            ...
            <div className="flex items-center gap-1">
              <button ref={toggleRef} onClick={() => setShowSearch((p) => !p)} className={`hover:text-white transition-all p-1 ${showSearch ? 'text-red-600' : 'text-zinc-500'}`}>
        ...
        {/* SEARCH AREA - Juga dibungkus agar sejajar */}
        <AnimatePresence>
          {showSearch && (
            <motion.div initial={{ height: 0 }} animate={{ height: "auto" }} exit={{ height: 0 }} className="overflow-hidden bg-black px-2 md:px-4 pb-4">
              <div ref={searchRef} className="relative">
                <input value={queryValue} onChange={handleSearch} placeholder="Search BOSS..." className="w-full bg-zinc-950 border border-zinc-700 rounded-lg px-10 py-2.5 text-white text-[10px] font-mono tracking-widest" />
      ...
      {/* TABLE HEADER - Sudah memiliki pembungkus internal max-w-5xl */}
      <TableHeader />
